package com.example.note_app;

import android.os.Bundle;

import androidx.fragment.app.Fragment;

import android.view.LayoutInflater;
import android.view.View;
import android.view.ViewGroup;

import com.example.note_app.databinding.FragmentAddNoteBinding;

/**
 * A simple {@link Fragment} subclass.
 * Use the {@link Add_Note#newInstance} factory method to
 * create an instance of this fragment.
 */
public class Add_Note extends Fragment {

    FragmentAddNoteBinding binding;


    // TODO: Rename parameter arguments, choose names that match
    // the fragment initialization parameters, e.g. ARG_ITEM_NUMBER
    private static final String ARG_PARAM1 = "param1";
    private static final String ARG_PARAM2 = "param2";

    // TODO: Rename and change types of parameters
    private String mParam1;
    private String mParam2;

    public Add_Note() {
        // Required empty public constructor
    }

    /**
     * Use this factory method to create a new instance of
     * this fragment using the provided parameters.
     *
     * @param param1 Parameter 1.
     * @param param2 Parameter 2.
     * @return A new instance of fragment Add_Note.
     */
    // TODO: Rename and change types and number of parameters
    public static Add_Note newInstance(String param1, String param2) {
        Add_Note fragment = new Add_Note();
        Bundle args = new Bundle();
        args.putString(ARG_PARAM1, param1);
        args.putString(ARG_PARAM2, param2);
        fragment.setArguments(args);
        return fragment;
    }

    @Override
    public void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        if (getArguments() != null) {
            mParam1 = getArguments().getString(ARG_PARAM1);
            mParam2 = getArguments().getString(ARG_PARAM2);
        }
        binding = FragmentAddNoteBinding.inflate(getLayoutInflater());

        binding.btncancle.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                binding.AddNoteBodyInput.setText("");
                binding.AddNoteTitleInput.setText("");
            }
        });
        binding.btnsave.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                String title = binding.AddNoteTitleInput.getText().toString();
                String body = binding.AddNoteBodyInput.getText().toString();
                MyDBHalper dbHalper = new MyDBHalper(getContext());
                dbHalper.insert_note(title,body);
                dbHalper.close();
            }
        });

    }

    @Override
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
                             Bundle savedInstanceState) {
        // Inflate the layout for this fragment
        return binding.getRoot();
    }
}